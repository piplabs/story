...
func CreateUpgradeHandler(
	...
	return func(ctx context.Context, _ upgradetypes.Plan, vm module.VersionMap) (module.VersionMap, error) {
		...
		aKeeper := keepers.AccountKeeper
		sKeeper := keepers.StakingKeeper
		dKeeper := keepers.DistrKeeper
		...
		if err != nil {
			return vm, lerrors.Wrap(err, "get staking params")
		}
		...
		if stakingParams.TokenTypes[0].TokenType != lockedTokenType {
			return vm, lerrors.New("locked token type not found in token types", "expected", lockedTokenType, "found", stakingParams.TokenTypes[0].TokenType)
		}
		stakingParams.TokenTypes[0].RewardsMultiplier = NewLockedTokenMultiplier

		if err := sKeeper.SetParams(ctx, stakingParams); err != nil {
			return vm, lerrors.Wrap(err, "update staking params")
		}
		...
		if err != nil {
			return vm, lerrors.Wrap(err, "get all validators")
		}
		...
		if oldTotalRewardsTokens.IsZero() {
			return vm, lerrors.New("old total rewards tokens is zero")
		}
		if !oldTotalRewardsTokensOfLockedValidators.Add(oldTotalRewardsTokensOfUnlockedValidators).Equal(oldTotalRewardsTokens) {
			return vm, lerrors.New("old total rewards tokens of locked validators and unlocked validators do not sum to old total rewards tokens")
		}
		...
		for _, val := range validators {
			...
			if err != nil {
				return vm, lerrors.Wrap(err, "failed to convert validator bech32 address to validator address", "validator_address", val.OperatorAddress)
			}

			// get all delegations
			dels, err := sKeeper.GetValidatorDelegations(ctx, valAddr)
			if err != nil {
				return vm, lerrors.Wrap(err, "failed to get all delegations for the validator")
			}
			...
			for _, del := range dels {
				...
				if err := sKeeper.SetDelegation(ctx, del); err != nil {
					return vm, lerrors.Wrap(err, "failed to set fixed delegation")
				}

				// verify after set
				delAddr, err := aKeeper.AddressCodec().StringToBytes(del.DelegatorAddress)
				if err != nil {
					return vm, lerrors.Wrap(err, "failed to convert bech32 delegator address from string")
				}

				updatedDel, err := sKeeper.GetDelegation(ctx, sdk.AccAddress(delAddr), valAddr)
				if err != nil {
					return vm, lerrors.Wrap(err, "failed to verify updated delegation")
				}

				if !updatedDel.RewardsShares.Equal(newDelRewardsShares) {
					return vm, lerrors.New(fmt.Sprintf("delegation rewards_shares fix verification failed: expected=%s got=%s",
				...
				if err != nil {
					return vm, lerrors.Wrap(err, "failed to get period delegation",
				...
				if err := sKeeper.SetPeriodDelegation(ctx, sdk.AccAddress(delAddr), valAddr, periodDel); err != nil {
					return vm, lerrors.Wrap(err, "failed to set updated period delegation",
						"validator_address", valAddr.String(),
						"delegator_address", sdk.AccAddress(delAddr).String(),
					)
				}
				...
				if err != nil {
					return vm, lerrors.Wrap(err, "failed to get updated period delegation",
						"validator_address", valAddr.String(),
						"delegator_address", sdk.AccAddress(delAddr).String(),
					)
				}
				...
				if !updatedPeriodDel.RewardsShares.Equal(newDelRewardsShares) {
					return vm, lerrors.New(fmt.Sprintf("period delegation rewards_shares fix verification failed: expected=%s got=%s",
			...
			if err := sKeeper.SetValidator(ctx, val); err != nil {
				return vm, lerrors.Wrap(err, "update validator rewards tokens", "validator", val.OperatorAddress)
			}

			// validation
			newVal, err := sKeeper.GetValidator(ctx, valAddr)
			if err != nil {
				return vm, lerrors.Wrap(err, "failed to get validator")
			}

			if !newVal.DelegatorRewardsShares.Equal(newTotalRewardsShares) {
				return vm, lerrors.New("sum of delegators' rewards shares does not equal to the expected one",
					"validator_address", valAddr.String(),
			...
			if !newVal.RewardsTokens.Equal(newRewardsTokens) {
				return vm, lerrors.New("rewards tokens of the validator does not equal to the expected one",
					"validator_address", valAddr.String(),
			...
			if err != nil {
				return vm, lerrors.Wrap(err, "failed to get all updated delegations for the validator")
			}
			...
			for _, del := range updatedDels {
				...
				if err != nil {
					return vm, lerrors.Wrap(err, "failed to get delegator starting info")
				}
				...
				if err := dKeeper.SetDelegatorStartingInfo(ctx, valAddr, sdk.AccAddress(delAddr), startingInfo); err != nil {
					return vm, lerrors.Wrap(err, "failed to set delegator starting info")
				}

				scaledDelegatorStartingInfo, err := dKeeper.GetDelegatorStartingInfo(ctx, valAddr, sdk.AccAddress(delAddr))
				if err != nil {
					return vm, lerrors.Wrap(err, "failed to get scaled delegator starting info")
				}

				if !scaledDelegatorStartingInfo.RewardsStake.Equal(newRewardsStake) {
					return vm, lerrors.New("delegator rewards stake mismatch",
						"validator_address", valAddr.String(),
						"delegator_address", sdk.AccAddress(delAddr).String(),
		...
		mintParams, err := keepers.MintKeeper.GetParams(ctx)
		if err != nil {
			return vm, lerrors.Wrap(err, "get mint params")
		}
		...
		if err := mintParams.Validate(); err != nil {
			return vm, lerrors.Wrap(err, "validate mint params")
		}

		if err := keepers.MintKeeper.SetParams(ctx, mintParams); err != nil {
			return vm, lerrors.Wrap(err, "update mint params")
		}

		// check that the annual inflations per year is updated
		mintParams, err = keepers.MintKeeper.GetParams(ctx)
		if err != nil {
			return vm, lerrors.Wrap(err, "reload mint params")
		}
