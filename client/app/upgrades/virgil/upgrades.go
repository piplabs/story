...
func CreateUpgradeHandler(
	...
	return func(ctx context.Context, _ upgradetypes.Plan, vm module.VersionMap) (module.VersionMap, error) {
		...
		if err := runVirgilUpgrade(ctx, keepers.StakingKeeper, chainID); err != nil {
			return vm, err
		}
...
func runVirgilUpgrade(ctx context.Context, sKeeper StakingKeeper, chainID string) error {
	...
	if err := sKeeper.SetParams(ctx, stakingParams); err != nil {
		return errors.Wrap(err, "failed to update staking params")
	}
	...
	periodDelegations, err := sKeeper.GetAllPeriodDelegations(ctx)
	if err != nil {
		return errors.Wrap(err, "failed to get all period delegations")
	}
	...
	for i := range periodDelegations {
		...
		if err != nil {
			return errors.Wrap(err, "failed to get delegator address")
		}
		...
		if err := sKeeper.SetPeriodDelegation(ctx, delAddr, valAddr, periodDelegations[i]); err != nil {
			return errors.Wrap(err, "failed to set period delegation")
		}

		newPeriodDelegation, err := sKeeper.GetPeriodDelegation(ctx, delAddr, valAddr, periodDelegations[i].PeriodDelegationId)
		if err != nil {
			return errors.Wrap(err, "failed to get new period delegation")
		}
		...
		if newPeriodDelegation.EndTime != newEndTime {
			return errors.New("new period delegation end time is not correct")
		}
	}
