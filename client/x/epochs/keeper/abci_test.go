...
func (suite *KeeperTestSuite) TestEpochInfoBeginBlockChanges() {
	...
	for name, test := range tests {
		suite.Run(name, func() {
			...
			// get sorted heights
			var heights []int
			for h := range test.blockHeightTimePairs {
				heights = append(heights, h)
			}
			sort.Slice(heights, func(i, j int) bool {
				return test.blockHeightTimePairs[heights[i]].Before(test.blockHeightTimePairs[heights[j]])
			})
...
func TestEpochStartingOneMonthAfterInitGenesis(t *testing.T) {
	...
	require.Equal(t, initialBlockHeight, epochInfo.CurrentEpochStartHeight)
	...
	epochInfo, err = epochsKeeper.EpochInfo.Get(ctx, "monthly")
	require.NoError(t, err)
	require.Equal(t, int64(1), epochInfo.CurrentEpoch)
	require.Equal(t, ctx.BlockHeight(), epochInfo.CurrentEpochStartHeight)
	require.Equal(t, now.Add(month).UTC().String(), epochInfo.CurrentEpochStartTime.UTC().String())
