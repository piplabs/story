...
func (k *Keeper) ProcessCDRVaultRead(ctx context.Context, ethlog *ethtypes.Log) (err error) {
	...
	if err = k.dkgKeeper.ThresholdDecryptRequested(cachedCtx, latestActive.Round, ev.RecipientPublicKey, ev.EncryptedData, label); errors.Is(err, sdkerrors.ErrInvalidRequest) {
