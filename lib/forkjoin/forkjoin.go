...
// It is similar to errgroups, except that each "group" doesn't only return an error, but also a result.
//
// This package was copied from Obol's Charon repo.
// See https://github.com/ObolNetwork/charon/blob/main/app/forkjoin/forkjoin.go.
package forkjoin
...
// results will contain context canceled errors.
func (r Results[I, O]) Flatten() ([]O, error) {
	//nolint:prealloc // We don't know the length of the results.
	var (
		ctxErr   error
		otherErr error
		resp     []O
...
// The default behavior just cancels the worker context and closes the output channel without waiting
...
// are canceled and no further inputs are executed with remaining result errors being set
// to context canceled. See WithoutFailFast.
//
// Usage:
//
