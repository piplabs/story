...
func NewFireBackends(ctx context.Context, testnet types.Testnet, fireCl fireblocks.Client) (Backends, error) {
	...
	// Configure iliad EVM Backend
	if testnet.HasIliadEVM() {
		chain := testnet.BroadcastIliadEVM()
		...
		if err != nil {
			return Backends{}, errors.Wrap(err, "new iliad Backend")
		}
	}
...
func NewBackends(testnet types.Testnet, deployKeyFile string) (Backends, error) {
	...
	// Configure iliad EVM Backend
	{
		...
		// dev iliad evm uses same dev accounts as anvil
		// TODO: do not use dev anvil backend for prod iliad evms
