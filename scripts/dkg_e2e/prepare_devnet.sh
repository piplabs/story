...
#   1. Switches ALL machines to dkg/dev latest
#   2. Applies devnet sed patches (V160 fork height, DKG owner, SGXValidationHook owner, validator allocs)
#   3. Gets kernel mrenclave and updates SGX_CODE_COMMITMENT on ALL machines
#   4. Generates EL alloc on val1 (forge script)
#   5. Builds EL genesis + updates CL genesis execution_block_hash
#   6. Builds story binary on val1
#   7. Distributes binary + genesis to all machines
#
# After this script, all machines have:
#   - dkg/dev source + devnet sed patches (real SGXValidationHook, short periods, EOA DKG owner)
...
# --- Step 1: Switch ALL machines to dkg/dev latest ---
echo "=== Step 1: Switching all machines to dkg/dev ==="
...
echo "  bootnode: dkg/dev"

# --- Step 1.1: Apply devnet sed patches on ALL machines ---
# These replace the old cherry-pick (c61b74a4) with targeted sed commands.
echo "=== Step 1.1: Applying devnet patches ==="

V160_FORK_HEIGHT="${DKG_V160_FORK_HEIGHT:-105}"
ALLOC_FILE="${STORY_SRC}/contracts/script/GenerateAlloc.s.sol"
V160_CONST="${STORY_SRC}/client/app/upgrades/v_1_6_0/constants.go"

# Patch 1: V160 fork height (so DKG activates at block 105)
# Patch 2: DKG owner = TEST_DKG_OWNER (Foundry test key, for whitelistEnclaveType)
# Patch 4: SGXValidationHook owner = TEST_DKG_OWNER (real hook, just EOA owner)
# Patch 5: Devnet validator ETH allocations
# Note: remote machines are Linux (sed -i without '' suffix)
DEVNET_PATCH_CMD="cd ${STORY_SRC} && \
  sed -i 's/BeginForkLogic: func/UpgradeHeight:  ${V160_FORK_HEIGHT},\n\tBeginForkLogic: func/' ${V160_CONST} && \
  sed -i '/KEEP_TIMELOCK_ADMIN_ROLE = false/a\\
\\n    uint256 private constant TEST_DKG_OWNER_KEY = 0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80;\\n    address private constant TEST_DKG_OWNER = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266;' ${ALLOC_FILE} && \
  sed -i '/DKG(Predeploys.DKG).initialize(/,/);/ s/timelock,/TEST_DKG_OWNER,/' ${ALLOC_FILE} && \
  sed -i 's/vm.prank(timelock);/vm.prank(TEST_DKG_OWNER);/' ${ALLOC_FILE} && \
  sed -i 's/(timelock, AUTOMATA_VALIDATION_ADDR/(TEST_DKG_OWNER, AUTOMATA_VALIDATION_ADDR/' ${ALLOC_FILE} && \
  sed -i 's/abi.encode(sgxHookImpl, timelock, initData)/abi.encode(sgxHookImpl, TEST_DKG_OWNER, initData)/' ${ALLOC_FILE} && \
  sed -i '/0x64a2fdc6f7CD8AA42e0bb59bf80bC47bFFbe4a73/a\\
            ...
            vm.deal(0xcd29b70ff04C0aa386F7b3453dF0E5eD3d4F67bb, 100000000 ether);' ${ALLOC_FILE} && \
  echo 'devnet patches applied'"

for i in $(seq 1 "$TOTAL"); do
  RESULT=$(_ssh_cmd "$i" "$DEVNET_PATCH_CMD" 2>&1 | tail -1)
  echo "  node $i: ${RESULT}"
done
RESULT=$(ssh ${DKG_SSH_KEY:+-i "$DKG_SSH_KEY"} ${DKG_SSH_OPTS:-} ${BOOTNODE_USER}@${BOOTNODE_IP} "$DEVNET_PATCH_CMD" 2>&1 | tail -1 || echo "patch failed")
echo "  bootnode: ${RESULT}"

# --- Step 1.5: Patch short DKG periods in params.go on all machines ---
# V160 upgrade handler calls SetParams(DefaultParams()), so we patch the default values.
REG_PERIOD="${DKG_SHORT_REGISTRATION:-50}"
DEAL_PERIOD="${DKG_SHORT_DEALING:-80}"
FIN_PERIOD="${DKG_SHORT_FINALIZATION:-80}"
ACTIVE_PERIOD="${DKG_SHORT_ACTIVE:-20}"
MIN_REG="${DKG_SHORT_MIN_REG:-3}"
MIN_FIN="${DKG_SHORT_MIN_FIN:-3}"
PARAMS_FILE="${STORY_SRC}/client/x/dkg/types/params.go"

echo "=== Step 1.5: Patching DKG periods (reg=${REG_PERIOD} deal=${DEAL_PERIOD} fin=${FIN_PERIOD} active=${ACTIVE_PERIOD} min_reg=${MIN_REG} min_fin=${MIN_FIN}) ==="

PATCH_PERIOD_CMD="cd ${STORY_SRC} && \
  sed -i 's/DefaultDkgRegistrationPeriod uint32 = .*/DefaultDkgRegistrationPeriod uint32 = ${REG_PERIOD}/' ${PARAMS_FILE} && \
  sed -i 's/DefaultDkgDealingPeriod      uint32 = .*/DefaultDkgDealingPeriod      uint32 = ${DEAL_PERIOD}/' ${PARAMS_FILE} && \
  sed -i 's/DefaultDkgFinalizationPeriod uint32 = .*/DefaultDkgFinalizationPeriod uint32 = ${FIN_PERIOD}/' ${PARAMS_FILE} && \
  sed -i 's/DefaultDkgActivePeriod       uint32 = .*/DefaultDkgActivePeriod       uint32 = ${ACTIVE_PERIOD}/' ${PARAMS_FILE} && \
  sed -i 's/DefaultMinReqRegisteredParticipants uint32 = .*/DefaultMinReqRegisteredParticipants uint32 = ${MIN_REG}/' ${PARAMS_FILE} && \
  sed -i 's/DefaultMinReqFinalizedParticipants  uint32 = .*/DefaultMinReqFinalizedParticipants  uint32 = ${MIN_FIN}/' ${PARAMS_FILE}"
...
# Force chain_id to LocalChainID (must match UpgradeHistories in code)
old_cid = cl_genesis.get('chain_id', '')
cl_genesis['chain_id'] = 'story-1001511'
if old_cid != 'story-1001511':
    print(f'Fixed chain_id: {old_cid} -> story-1001511')
...
# Helper: stop story process and wait until it is confirmed dead.
# Usage: _stop_and_wait_story <node_index>
# For bootnode, use _stop_and_wait_story_bootnode instead.
_stop_and_wait_story() {
  local idx="$1"
  _ssh_cmd "$idx" "
    sudo systemctl stop story story-kernel node-geth 2>/dev/null || true
    sudo pkill -9 -x story 2>/dev/null || true
    for attempt in \$(seq 1 10); do
      pgrep -x story >/dev/null 2>&1 || { echo 'story stopped'; exit 0; }
      echo \"  waiting for story to die (attempt \$attempt)...\"
      sudo pkill -9 -x story 2>/dev/null || true
      sleep 1
    done
    echo '[ERROR] story process still alive after 10 attempts'
    exit 1
  "
}

_stop_and_wait_story_bootnode() {
  ssh ${DKG_SSH_KEY:+-i "$DKG_SSH_KEY"} ${DKG_SSH_OPTS:-} ${BOOTNODE_USER}@${BOOTNODE_IP} "
    sudo systemctl stop story node-geth 2>/dev/null || true
    sudo pkill -9 -x story 2>/dev/null || true
    for attempt in \$(seq 1 10); do
      pgrep -x story >/dev/null 2>&1 || { echo 'story stopped'; exit 0; }
      echo \"  waiting for story to die (attempt \$attempt)...\"
...
_stop_and_wait_story 1
_ssh_cmd 1 "sudo rm -f /usr/local/bin/story && sudo cp ${STORY_SRC}/build/story /usr/local/bin/story && story version 2>&1 | head -1"
...
for i in $(seq 2 "$TOTAL"); do
  ...
  _stop_and_wait_story "$i"
  scp ${DKG_SSH_KEY:+-i "$DKG_SSH_KEY"} ${DKG_SSH_OPTS:-} /tmp/story-dkg-dev ${target}:/tmp/story-new
  _ssh_cmd "$i" "sudo rm -f /usr/local/bin/story && sudo cp /tmp/story-new /usr/local/bin/story && rm /tmp/story-new && story version 2>&1 | head -1"
...
ssh ${DKG_SSH_KEY:+-i "$DKG_SSH_KEY"} ${DKG_SSH_OPTS:-} ${BOOTNODE_USER}@${BOOTNODE_IP} "sudo rm -f /usr/local/bin/story && sudo cp /tmp/story-new /usr/local/bin/story && rm /tmp/story-new"
...
echo "  All machines: dkg/dev + sed patches"
echo "=========================================="
echo ""
