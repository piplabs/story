...
if [ "$NEED_REBUILD" = "true" ]; then
  ...
  PULL_STORY_CMD="cd ${STORY_SRC} && git fetch origin ${STORY_BRANCH} && git reset --hard origin/${STORY_BRANCH}"
  ...
  # --- Patch DKG default params (V160 uses DefaultParams from params.go) ---
  echo "[configure_periods] Patching DKG default params on all machines..."
