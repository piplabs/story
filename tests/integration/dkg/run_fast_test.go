...
func TestDKG_FastSuite(t *testing.T) {
	...
	if !ok {
		t.Fatal("FastSuite requires Driver implementing PeriodConfigurator (set DKG_DRIVER=script)")
	}
	...
	if tier123NeedRun == 0 {
		...
		t.Logf("=== FastSuite Phase 1: %d Tier 1-3 tests need to run, configuring short periods ===", tier123NeedRun)
	...
	if len(upgrade) > 0 {
		...
		if upgradeAllPassed {
			t.Log("=== FastSuite Phase 3: All upgrade tests passed, skipping period switch ===")
		} else {
			upgradeReg := getUint64Env("DKG_UPGRADE_REGISTRATION", 50)
			if !periodsMatch(ctx, globalHarness, upgradeReg, shortDeal, shortFin, shortActive) {
				t.Logf("=== FastSuite Phase 3: Configure upgrade periods (reg=%d) ===", upgradeReg)
				if err := configurator.SetDKGPeriods(ctx, upgradeReg, shortDeal, shortFin, shortActive); err != nil {
