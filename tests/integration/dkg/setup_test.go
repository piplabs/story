...
// WaitForActiveRound waits until there is an active DKG round with a GlobalPublicKey.
// Returns the active network or nil on timeout. Use this in CDR/DEC tests that need Active stage.
func (h *Harness) WaitForActiveRound(t *testing.T) *dkgtypes.DKGNetwork {
	t.Helper()
	ctx := context.Background()
	deadline := time.Now().Add(h.MaxWait)

	for time.Now().Before(deadline) {
		// Check current round first — most reliable
		cur, _ := h.GetLatestDKGNetwork(ctx)
		if cur != nil && cur.Stage == dkgtypes.DKGStageActive && len(cur.GlobalPublicKey) > 0 {
			return cur
		}
		// Also check latest active round, but only if stage is still Active
		net, err := h.GetLatestActiveDKGNetwork(ctx)
		if err == nil && net != nil && net.Stage == dkgtypes.DKGStageActive && len(net.GlobalPublicKey) > 0 {
			return net
		}
		if cur != nil {
			fmt.Fprintf(os.Stderr, "  … round=%d stage=%s → waiting for DKG_STAGE_ACTIVE\n", cur.Round, cur.Stage)
		}
...
// WaitForCDRPartials polls until partials appear for the given uuid+requesterPubKey, or timeout.
func (h *Harness) WaitForCDRPartials(t *testing.T, uuid uint32, requesterPubKeyHex string, timeout time.Duration) *dkgtypes.QueryGetCDRPartialsResponse {
	t.Helper()
	ctx := context.Background()
	deadline := time.Now().Add(timeout)
	for time.Now().Before(deadline) {
		resp, err := h.GetCDRPartials(ctx, uuid, requesterPubKeyHex)
		if err == nil && len(resp.Submissions) > 0 {
			return resp
		}
		time.Sleep(5 * time.Second)
	}
