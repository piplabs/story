...
func AllScenarios() []Scenario {
	return []Scenario{
		...
		{
			Name:         "story_crash_one_validator",
			CaseIDs:      []string{"CL-CRASH-01"},
			SetupDesc:    "停 validator 2 的 story（systemctl stop story）→ 其他 2 个继续出块 → 重启 → 追上 → 参与 DKG。",
			TeardownDesc: "重启 validator 2 的 story + 等待 30s 追上。",
		},
		{
			Name:         "all_nodes_crash",
			CaseIDs:      []string{"CL-CRASH-02"},
			SetupDesc:    "停全部 3 个 story + kernel → 重启 → 验证共识恢复 + DKG 继续。",
			TeardownDesc: "重启所有 story + kernel。",
		},
	}
}
...
func AllMockKernelScenarios() []Scenario {
	return []Scenario{
		...
		{
			Name:         "kernel_reconnect",
			CaseIDs:      []string{"CL-RECONNECT-01"},
			SetupDesc:    "停 validator 3 的 kernel（不停 story）→ story 报 kernel unavailable → 重启 kernel → story 自动重连 → 下一轮正常注册。",
			TeardownDesc: "重启 validator 3 的 kernel。",
		},
		// kernel_restart_active_decrypt: 不再使用 pre/post 脚本。
		// 停/启 kernel 在 test 函数内部完成（需要先等 Active + CDR Read，然后才停 kernel）。
	}
}
