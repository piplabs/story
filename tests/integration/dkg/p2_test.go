...
func P2Cases() []TestCase {
	...
	decPriority := map[int]string{1: "P0", 16: "P0"} // rest default P1
	for i := 1; i <= 22; i++ {
		...
		// Add NeedsRoundWait for BLOCKED CDR/DEC test IDs
		if i == 1 || i == 5 || i == 6 || i == 7 || i == 8 || i == 9 || i == 10 || i == 11 || i == 12 || i == 13 || i == 14 || i == 15 || i == 17 || i == 18 || i == 19 || i == 20 || i == 21 || i == 22 {
			tc.NeedsRoundWait = true
		}
		list = append(list, tc)
	}
	// IT-CDR-04 .. IT-CDR-08
	list = append(list,
		TestCase{ID: "IT-CDR-04", Priority: "P1", Description: "FeeCollected amount=0: no AddCDRFeeToPool", Expected: "zero-fee event path", NeedsRoundWait: true, Run: runIT_CDR_04},
		TestCase{ID: "IT-CDR-05", Priority: "P1", Description: "Pool balance < refund amount: error", Expected: "pool underflow error", NeedsRoundWait: true, Run: runIT_CDR_05},
		TestCase{ID: "IT-CDR-06", Priority: "P0", Description: "No previous active round: distributeCDRRewardPool returns nil", Expected: "first round has no previous active", Run: runIT_CDR_06},
		TestCase{ID: "IT-CDR-07", Priority: "P1", Description: "Total submit count=0, pool>0: no SendCoins", Expected: "zero-submit-count path, no SendCoins", NeedsRoundWait: true, Run: runIT_CDR_07},
		TestCase{ID: "IT-CDR-08", Priority: "P1", Description: "E2E: CDR write/read + partial decryption + distributeCDRRewardPool", Expected: "full CDR lifecycle executed", NeedsRoundWait: true, Run: runIT_CDR_08},
	)
	return list
}
...
func runIT_CDR_08(t *testing.T, h *Harness) {
	...
	// Wait for active round with GlobalPublicKey
...
func runIT_ENC_01(t *testing.T, h *Harness) {
	...
	if !h.IsNoopChainClient() {
		...
		checkTrue(t, "uuid > 0", uuid > 0, fmt.Sprintf("uuid=%d", uuid))
		encData := []byte("test-tdh2-encrypted-data-enc-01")
		if err := h.ChainClient.CDRWrite(ctx, uuid, encData); err != nil {
			t.Fatalf("CDRWrite: %v", err)
		}
		t.Logf("IT-ENC-01: CDR.write succeeded: uuid=%d, data=%d bytes", uuid, len(encData))
		return
	}
...
func runIT_DEC_01(t *testing.T, h *Harness) {
	...
	net := h.WaitForActiveRound(t)
	if net == nil {
		t.Skip("no active round after waiting (ProcessCDRVaultRead requires latestActive)")
		return
	}
	checkTrue(t, "GlobalPublicKey", len(net.GlobalPublicKey) > 0, fmt.Sprintf("len=%d", len(net.GlobalPublicKey)))
	// With EthChainClient, exercise full CDR path: Allocate→Write→Read→WaitPartials
	if !h.IsNoopChainClient() {
		uuid, err := h.ChainClient.CDRAllocate(ctx)
		if err != nil {
			t.Fatalf("CDRAllocate: %v", err)
		...
		// Wait for partial decryption responses
		requesterPubKeyHex := fmt.Sprintf("%x", requesterPubKey)
		resp := h.WaitForCDRPartials(t, uuid, requesterPubKeyHex, 120*time.Second)
		if resp != nil && len(resp.Submissions) > 0 {
			checkTrue(t, "partials received", len(resp.Submissions) > 0,
				fmt.Sprintf("submissions=%d", len(resp.Submissions)))
			t.Logf("IT-DEC-01: uuid=%d partials=%d (decrypt path verified)", uuid, len(resp.Submissions))
		} else {
			t.Logf("IT-DEC-01: uuid=%d CDR read path exercised (partials not available within timeout — active period may have ended)", uuid)
		}
		return
	}
