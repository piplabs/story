...
func ensureAllHealthy(t *testing.T) {
	...
	for i := 0; i < validatorCount(); i++ {
		...
		if strings.TrimSpace(out) != "active" {
			t.Logf("  validator %d kernel not active (%s), updating trusted block + restarting...", i+1, strings.TrimSpace(out))
...
// updateKernelTrustedBlock fetches the current chain's block 5 and updates the kernel
// config.toml with the correct trusted_height/trusted_hash. Also clears stale light client
// and key data so the kernel starts fresh.
func updateKernelTrustedBlock(nodeIndex int) {
	kernelHome := os.Getenv("DKG_KERNEL_HOME")
	if kernelHome == "" {
		kernelHome = "/home/ubuntu/.story-kernel"
	}
	// Get block 5 from validator 0 (which should have a working story node)
	blockJSON, _ := sshRunCmd(0, "curl -s 'http://localhost:26657/block?height=5'")
	if blockJSON == "" {
		return
	}
	// Use python to extract height and hash (avoids jq dependency issues)
	info, _ := sshRunCmd(0, fmt.Sprintf(`python3 -c "
import json
d=json.loads('''%s''')
h=d['result']['block']['header']['height']
hh=d['result']['block_id']['hash']
print(h + ' ' + hh)
"`, strings.ReplaceAll(blockJSON, "'", "\\'")))
	parts := strings.Fields(strings.TrimSpace(info))
	if len(parts) != 2 {
		return
	}
	height, hash := parts[0], parts[1]
	// Update config and clear stale data
	sshRunCmd(nodeIndex, fmt.Sprintf(
		"sudo systemctl stop story-kernel 2>/dev/null; "+
			"sed -i 's/^trusted_height = .*/trusted_height = %s/' %s/config.toml; "+
			"sed -i 's/^trusted_hash = .*/trusted_hash = \"%s\"/' %s/config.toml; "+
			"rm -rf %s/light_client/ %s/keys/ %s/dkg_state/",
		height, kernelHome, hash, kernelHome, kernelHome, kernelHome, kernelHome))
}
