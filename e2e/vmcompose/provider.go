...
func (p *Provider) Setup() error {
	...
	for vmIP, services := range groupByVM(p.Data.Instances) {
		// Get all iliad nodes in this VM
		var nodes []*e2e.Node
		var iliads []string
		for _, node := range p.Testnet.Nodes {
			if services[node.Name] {
				nodes = append(nodes, node)
				iliads = append(iliads, node.Name)
			}
		}

		var geths []string
		for _, iliadEVM := range p.Testnet.IliadEVMs {
			if services[iliadEVM.InstanceName] {
				geths = append(geths, iliadEVM.InstanceName)
			}
		}

		// Get all iliadEVMs in this VM
		var iliadEVMs []types.IliadEVM
		for _, iliadEVM := range p.Testnet.IliadEVMs {
			if services[iliadEVM.InstanceName] {
				iliadEVMs = append(iliadEVMs, iliadEVM)
			}
		}
		...
		hostname := vmIP // TODO: Add hostnames to infra instances.
		agentCfg = agent.ConfigForHost(agentCfg, hostname, iliads, geths)
...
func (p *Provider) Upgrade(ctx context.Context, cfg types.UpgradeConfig) error {
	...
	// Include iliad config
	for _, node := range p.Testnet.Nodes {
		addFile(node.Name, "config", "iliad.toml")
	...
	// Include geth config
	for _, iliadEVM := range p.Testnet.IliadEVMs {
		addFile(iliadEVM.InstanceName, "config.toml")
		addFile(iliadEVM.InstanceName, "geth", "jwtsecret")
		addFile(iliadEVM.InstanceName, "geth", "nodekey")
	}
		...
		eg.Go(func() error {
			...
			for service, filePaths := range filesByService {
				...
				for _, filePath := range filePaths {
					...
					remotePath := filepath.Join("/iliad", p.Testnet.Name, service, filePath)
			...
			remoteComposePath := filepath.Join("/iliad", p.Testnet.Name, composeFile)
			...
			if services := p.Data.ServicesByInstance(instance); containsEVM(services) {
				if containsAnvil(services) {
					return errors.New("cannot upgrade VM with both iliad_evm and anvil containers since iliad_evm needs downing and anvil cannot be restarted", "vm", vmName)
				}
				maybeDown = "sudo docker compose down && "
			}

			startCmd := fmt.Sprintf("cd /iliad/%s && "+
...
func (p *Provider) StartNodes(ctx context.Context, _ ...*e2e.Node) error {
	...
	p.once.Do(func() {
		...
		// TODO: Only start additional services and then start iliad as per above StartNodes.
		for vmName, instance := range p.Data.VMs {
			composeFile := vmComposeFile(instance.IPAddress.String())
...
func (p *Provider) Clean(ctx context.Context) error {
	...
	for vmName := range p.Data.VMs {
		for _, cmd := range docker.CleanCmds(true, true) {
			...
			err = execOnVM(ctx, vmName, "sudo rm -rf /iliad/*")
			if err != nil {
				return errors.Wrap(err, "clean docker containers", "vm", vmName)
			}
		}
...
func copyToVM(ctx context.Context, vmName string, path string) error {
	tarscp := fmt.Sprintf("tar czf - %s | gcloud compute ssh --zone=us-east1-c %s --quiet -- \"cd /iliad && tar xzf -\"", filepath.Base(path), vmName)
...
// containsEVM returns true if the services map contains an iliad evm.
