...
func WriteAllConfig(testnet types.Testnet, genesis core.Genesis) error {
	...
	gethConfigFiles := func(evm types.IliadEVM) map[string][]byte {
	...
	for _, evm := range testnet.IliadEVMs {
		...
		conf := Config{
			...
			BootNodes:    evm.Peers, // TODO: Use seed nodes once available.
...
// MakeGethConfig returns the full iliad geth config for the provided custom config.
func MakeGethConfig(conf Config) FullConfig {
	...
	cfg.Node.P2P.DiscoveryV4 = true // TODO: Switch to v5.
