...
import (
	...
	evmgenutil "github.com/storyprotocol/iliad/client/genutil/evm"
	"github.com/storyprotocol/iliad/e2e/app/agent"
	"github.com/storyprotocol/iliad/e2e/app/geth"
	"github.com/storyprotocol/iliad/e2e/app/static"
...
func Setup(ctx context.Context, def Definition, depCfg DeployConfig) error {
	...
	for _, node := range def.Testnet.Nodes {
		...
		iliadEVM := iliadEVMByPrefix(def.Testnet, node.Name)

		if err := writeIliadConfig(
			def.Testnet.Network,
			nodeDir,
			def.Cfg,
			logCfg,
			depCfg.testConfig,
			node.Mode,
			iliadEVM.InstanceName,
		); err != nil {
			return err
		}

		if err := os.WriteFile(filepath.Join(nodeDir, "config", "jwtsecret"), []byte(iliadEVM.JWTSecret), 0o600); err != nil {
		...
		initCfg := iliadcmd.InitConfig{
			HomeDir: nodeDir,
			Network: def.Testnet.Network,
		}
		if err := iliadcmd.InitFiles(log.WithNoopLogger(ctx), initCfg); err != nil {
...
func MakeConfig(node *e2e.Node, nodeDir string) (*config.Config, error) {
	cfg := iliadcmd.DefaultCometConfig(nodeDir)
...
// writeIliadConfig generates an iliad application config for a node and writes it to disk.
func writeIliadConfig(
	network netconf.ID,
	nodeDir string,
	...
	cfg.EngineJWTFile = "/client/config/jwtsecret"                  // Absolute path inside docker container
	...
	return iliadcfg.WriteConfigTOML(cfg, logCfg)
}
