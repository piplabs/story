// Package eoa defines well-known (non-fireblocks) eoa private keys used in an iliad network.
package eoa
...
const (
	...
	// RoleDeployer is used to deploy official iliad contracts on all chains.
	RoleDeployer Role = "deployer"
	// RoleAdmin is used to manage the iliad contracts on all chains. It has admin privileges on official iliad contracts.
...
// Account defines a EOA account used within the iliad network.
type Account struct {
	Type       Type
	Role       Role
