...
deploy_helpers() {
    ...
    # Skip if helpers are already deployed (pre-deployed in genesis alloc).
    local helpers_json="${PCCS_REPO}/deployment/${CHAIN_ID}.json"
    if [ -f "$helpers_json" ]; then
        local all_exist=true
        for name in EnclaveIdentityHelper FmspcTcbHelper PCKHelper X509CRLHelper TcbEvalHelper; do
            local addr
            addr=$(jq -r ".\"$name\"" "$helpers_json" 2>/dev/null)
            if [ -z "$addr" ] || [ "$addr" = "null" ]; then all_exist=false; break; fi
            local code
            code=$(cast code "$addr" --rpc-url "$DEVNET_RPC_URL" 2>/dev/null)
            if [ "$code" = "0x" ] || [ -z "$code" ]; then all_exist=false; break; fi
        done
        if [ "$all_exist" = true ]; then
            info "PCCS helpers already deployed (genesis alloc). Skipping deployment."
            cp "$helpers_json" "$DCAP_DEPLOY_DIR/onchain_pccs.json"
            for name in EnclaveIdentityHelper FmspcTcbHelper PCKHelper X509CRLHelper TcbEvalHelper; do
                addr=$(read_pccs_addr "$name")
                write_output "$name" "$addr"
                info "  $name: $addr (pre-deployed)"
            done
            return 0
        fi
    fi
