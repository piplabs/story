...
/// @title TransferOwnershipIPTokenStaking
/// @notice Generates json files with the timelocked operations to transfer the ownership of IPTokenStaking to the new timelock
/// This contract is Ownable2StepUpgradeable, so we need to accept ownership transfer from the new timelock in the next step.
contract TransferOwnershipIPTokenStaking is TimelockOperations {
    ...
    constructor() TimelockOperations("safe-migr-transfer-ownership-iptoken-staking") {
        ...
        // Not using Predeploys.Staking directly to help with `aeneid-test` scripts
