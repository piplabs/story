...
/// @notice Generates json files with the timelocked operations to transfer the ownership of half of the proxy admins to the new timelock
contract TransferOwnershipsProxyAdmin2 is TimelockOperations {

    TimelockController public newTimelock;

    address[] public from;

    constructor() TimelockOperations("safe-migr-transfer-ownerships-proxy-admin-2") {
        from = new address[](3);
